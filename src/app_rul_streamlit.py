...
if os.path.exists(TRAIN_META_PATH):
    ...
    st.warning("Training metadata not found. Using default placeholder features.")
...
LSTM_FEATS = TREE_FEATURE_COLS[:5]
SEQ_LEN = 30
...
def load_models():
    ...
    try:
        ...
        st.warning("Real models not found. Loading demo models instead.")
...
def explain_model(model, X_sample, method="shap"):
    with st.spinner(f"Computing {method.upper()} feature importance... please wait"):
        try:
            if method == "shap":
                ...
                result = permutation_importance(model, X_sample, y_pred, n_repeats=5, random_state=42, n_jobs=-1)
...
explain_method = st.sidebar.radio("Model Explainability Method", ["None", "SHAP", "Permutation"], index=0)
...
if uploaded_file is not None:
    try:
        ...
        if df.empty:
            st.warning("The uploaded file is empty or not a valid CSV format.")
            st.stop()
    ...
    st.info("No file uploaded. Loading sample test data instead.")
    ...
    if os.path.exists(sample_path):
        try:
            ...
            st.error(f"Failed to load sample test data: {str(e)}")
            st.stop()
    else:
...
if df is not None:
    ...
    if st.button("Run Prediction"):
        try:
            ...
            elif explain_method == "Permutation":
                st.subheader("Model Explainability (Permutation Importance)")
                X_sample = df[TREE_FEATURE_COLS].sample(n=min(200, len(df)), random_state=42)
